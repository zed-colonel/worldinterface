...
use worldinterface_flowspec::payload::{CoordinatorPayload, StepPayload, TaskType};
...
mod tests {
    ...
    use worldinterface_core::flowspec::{ConnectorNode, NodeType};
    use worldinterface_core::id::{FlowRunId, NodeId};
    use worldinterface_core::metrics::NoopMetricsRecorder;
    use worldinterface_flowspec::payload::{StepPayload, TaskType};
    ...
    fn routes_coordinator_payload() {
        ...
        let fr = FlowRunId::new();
        let node_id = NodeId::new();
        let spec = worldinterface_core::flowspec::FlowSpec {
            id: None,
            name: None,
            nodes: vec![worldinterface_core::flowspec::Node {
                id: node_id,
                label: None,
                node_type: NodeType::Connector(ConnectorNode {
                    connector: "delay".into(),
        ...
        let task_id = worldinterface_flowspec::id::derive_task_id(fr, node_id);
        let mut node_task_map = std::collections::HashMap::new();
        node_task_map.insert(node_id, task_id);

        let payload = worldinterface_flowspec::payload::CoordinatorPayload {
