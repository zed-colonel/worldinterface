...
use worldinterface_flowspec::payload::CoordinatorPayload;
...
pub(crate) fn derive_flow_run_status(
    ...
    store: &dyn worldinterface_contextstore::ContextStore,
...
fn collect_step_info(
    ...
    for node in &coordinator_payload.flow_spec.nodes {
        ...
        let connector = match &node.node_type {
            worldinterface_core::flowspec::NodeType::Connector(c) => Some(c.connector.clone()),
            _ => None,
        };
...
pub(crate) fn extract_flow_outputs(
    store: &dyn worldinterface_contextstore::ContextStore,
    flow_run_id: FlowRunId,
) -> Result<HashMap<NodeId, Value>, worldinterface_contextstore::error::ContextStoreError> {
