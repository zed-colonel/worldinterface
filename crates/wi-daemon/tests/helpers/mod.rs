...
use wi_daemon::metrics::PrometheusMetricsRecorder;
use wi_daemon::{AppState, DaemonConfig, SharedState, WiMetricsRegistry};
use wi_host::{EmbeddedHost, FlowPhase, FlowRunStatus};
...
pub async fn start_test_daemon_at_path(path: &std::path::Path) -> TestDaemon {
    ...
    let webhook_registry =
        wi_http_trigger::WebhookRegistry::load_from_store(host.context_store()).unwrap();
    let state: SharedState =
        Arc::new(AppState { host, webhook_registry: RwLock::new(webhook_registry), metrics });
    let router = wi_daemon::router::build_router(Arc::clone(&state));
